...
const bullets = [
  "למה קרן ההשתלמות היא כלי ההשקעה הכי חזק שיש לך - ואיך לא לזרוק אותו",
  "החישוב הפשוט שמראה כמה כסף את מפסידה בשנה",
  "ההבדל בין למשוך לבין להשאיר - בשפה שכולם מבינות",
  "איך לקבל החלטה שמתאימה בדיוק לחיים שלך",
];
...
export default function KerenHishtalmutPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await fetch("/api/guides/keren-hishtalmut-morot", {
        method: "POST",
    ...
    <main dir="rtl" style={{ minHeight: "100vh", background: "radial-gradient(ellipse at 50% 20%, #1535B5 0%, #060D3C 65%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "60px 1.5rem 80px" }}>
      ...
      <div style={{ maxWidth: 520, width: "100%", textAlign: "center" }}>
        ...
        <div style={{ display: "inline-block", background: "rgba(33,240,176,0.12)", border: "1px solid rgba(33,240,176,0.35)", borderRadius: 50, padding: "6px 20px", marginBottom: 24 }}>
          <span style={{ color: "#21F0B0", fontSize: "0.82rem", fontWeight: 700, letterSpacing: 1 }}>מדריך חינמי למורות</span>
        </div>

        {/* Title */}
        <h1 style={{ color: "#FFFFFF", fontSize: "clamp(1.6rem, 5vw, 2.3rem)", fontWeight: 900, lineHeight: 1.3, marginBottom: 16 }}>
          קרן ההשתלמות שלך
        </h1>
        <p style={{ color: "#21F0B0", fontWeight: 700, fontSize: "1.05rem", marginBottom: 12 }}>
          מקבלת יותר כל חודש - ומפסידה הרבה יותר בטווח הארוך?
        </p>
        <p style={{ color: "rgba(255,255,255,0.7)", fontSize: "0.95rem", marginBottom: 36, lineHeight: 1.7 }}>
          הגיע הזמן להבין אחת ולתמיד מה נכון ומתאים לך.
        </p>
